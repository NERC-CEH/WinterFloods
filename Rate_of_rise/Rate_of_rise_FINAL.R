...
for (i in seq_along(KeyDetails)){ # for each station
  ...
  KeyDetails_1stn <- KeyDetails_long %>% dplyr::filter(.data[[ID_col_KD]] == stn)
  ...
  N <- nrow(data_in)
  NRates <- diff(lubridate::year(range(data_in$DateTime)))*5
  ...
  for(k in seq_along(period_vec)){ # for each period
    ...
    eventtable_t2$ror_rank <- 
      (NR+ND+1) - rank(c(outtable_temp$ror,eventtable_t2$ror), ties.method="last")[NR+(1:ND)]
    #})
    outtable_temp$ror_rank <- 
      (NR+ND+1) - rank(c(outtable_temp$ror,eventtable_t2$ror), ties.method="last")[1:NR]
