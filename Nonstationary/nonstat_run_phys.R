...
# Info: Extra plotting functions for nonstationary flood frequency analysis, based on using physical covariates in nonstationary analysis
...
keyevents_filename <-"./Data/Nonstationary/KeyDetails_Long.csv" # key metadata, one event per row
keydetails_filename <- ".Data/Metadata/Master Station Listing.csv" # key metadata, one station per row

##### DATA #####
KeyEvents <- readr::read_csv(keyevents_filename )

KeyDetails <- readr::read_csv(keydetails_filename)
...
PlotFit <- function (dist,
                     ...
                     detrend.cov)
{
  # A version of the plotting function used in the nonstat package, needs key parameters
  # dist = distribution name
  # fit = results of fitting the relevant distribution/model
  # ret_periods = vector of return periods
  # all_data_raw = underlying AMAX data
  # CI = range of confidence interval (e.g. 95% is 0.05)
  # gaugeNumber = number of station for plots
  # gaugeName = name of station for plots
  # detrend.cov = dataframe of covariates, detrended
  ...
  fitType <- ifelse(loc == "~1" & scale == "~1", "Stationary", "Non-stationary")
  covsInfo <- paste0("Location: ",
                     as.character(loc),
                     "; Scale: ",
                     as.character(scale))
  all_data <- all_data_raw[, !names(all_data_raw) %in% c("WaterYear", "WaterYearIndex")]
  ...
  f <- ggplot(covDataReshaped, aes(x = Flow, y = value)) +
    ...
    labs(x = expression(paste("Flow (", m ^ 3, "/s)")), y = "Value") + ggtitle(plot1Title) + theme(plot.title = element_text(hjust = 0.5))
  ...
  statRLsData <- statRLsData[which(word(statRLsData$RP, 1, sep = "-") %in% ret_periods_sub), ]
  if (fit$condRLs[1, 1] == "The selected model is stationary. Conditional return levels are not available for stationary models.") {
    statRLsData$RP2 <- as.numeric(word(statRLsData$RP, 1, sep = "-"))
    ...
    g <- ggplot() + geom_segment(
      ...
      geom_hline(
        data = statRLsData,
        aes(yintercept = RL, color = RP),
        size = 1,
        linetype = "dashed"
      ) +
      scale_colour_brewer(name = "Return Period", palette = "Dark2") + labs(x = "Water year", y = expression(paste("Flow (", m ^
                                                                                                                     3, "/s)"))) +
    ...
    CRLsData <- CRLsData[, grep("Index",
                                names(CRLsData),
                                value = TRUE,
                                invert = TRUE)]
    condRLsData <- melt(CRLsData, id.var = "WaterYear")
    names(condRLsData)[2] <- "RP"
    condRLsData <- condRLsData[which(word(condRLsData$RP, 1, sep = "-") %in% ret_periods_sub), ]
    plot2Title <- ifelse(
      nchar(gaugeName) > 0,
      paste0(
        "Gauging station ",
    ...
    g <- ggplot() + geom_segment(
      ...
      geom_point(data = condRLsData, aes(x = WaterYear, y = value, color = RP)) + geom_hline(
        data = statRLsData,
        aes(yintercept = RL, color = RP),
        size = 1,
        linetype = "dashed"
  ...
  if (fit$margRLs[1, 1] == "The selected model is stationary. Marginal return levels are not available for stationary models.") {
    ...
    statRLsData$variable2[statRLsData$variable2 %in% c("StatLCI", "StatUCI")] <- paste0(CI, "% confidence interval")
    statRLsData$variable2 <- as.factor(statRLsData$variable2)
    statRLsData$variable2 <- factor(statRLsData$variable2, levels(statRLsData$variable2)[c(3, 2, 1)])
    h <- ggplot() + geom_line(data = statRLsData,
                              aes(
                                x = EP,
                                y = value,
                                size = variable,
      ...
      labs(
        x = paste0("Encounter probability over the ", N, "-year record period"),
        y = expression(paste("Flow (", m ^ 3, "/s)"))
      ) + geom_rug(data = FlowData, aes(y = Flow)) + scale_size_manual(values = c(1, 1.5, 1)) + theme(legend.position = "none") +
    ...
    statRLs <- data.frame(RP = fit$statRLs$RP,
                          RL = fit$statRLs$Estimate)
    ...
    statMargData$variable2[statMargData$variable2 %in% c("MargLCI", "MargUCI")] <- paste0(CI, "% confidence interval")
    statMargData$variable2 <- as.factor(statMargData$variable2)
    statMargData$variable2 <- factor(statMargData$variable2, levels(statMargData$variable2)[c(3, 2, 1)])
    h <- ggplot() + geom_line(data = statMargData,
                              aes(
                                x = EP,
                                y = value,
                                size = variable,
      ...
      labs(
        x = paste0("Encounter probability over the ", N, "-year record period"),
        y = expression(paste("Flow (", m ^ 3, "/s)"))
      ) + geom_rug(data = FlowData, aes(y = Flow)) + scale_colour_manual(
        values = c("#000000", "#0072B2", "#0072B2"),
        labels = c(
          "Stationary flow estimates",
...
# key stations for analysis in the Winter Floods 2019-21 project
...
for(gauge in STN){ # for each station
  gauge <- as.character(gauge)
  RPs <- c(2, 10, 20, 50, 100)
  res.phys.cov(folder, physCovModelFits, gauge, RPs, CI,
                 model_type = "Best", locFit = NULL, scaleFit = NULL,
                 present_day = TRUE) # apply physical covariate analysis
}

for (k in seq_along(STN)) { # for each station
  #print(paste(k, STN[k]))
  ...
  for (fit in c("best_model", "stat_model")) { 
    # get additional model fitting information out of the nonstat package
    #print(paste("***", fit))
